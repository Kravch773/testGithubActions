...
import settlingInServicesPage from '../../../../pageobjects/assignment/services/settlingInServices.page';
import assignmentsPage from '../../../../pageobjects/clients/assignments.page';
import clientsPage from '../../../../pageobjects/clients/clients.page';

const serviceName = "Settling in services"
...
const purchaseOrderDetails_1 = ["Roebuck Removals ", "Settling in Services - 1 Day", "USD - US Dollar", "500", "testDescrpt1", startDate]
const purchaseOrderDetails_2 = ["Roebuck Removals", "Settling in Services - 1 Day", "EUR - Euro", "800", "descrpt2", commonElements.getCurrentDatePlusDaysNo0Format(8)]
const salesOrderDetails_1 = ["K2 Managment Fee - Settling In Service", "225", "descript1", startDate]
const salesOrderDetails_2 = ["K2 Managment Fee - Settling In Service", "565", "2descript", commonElements.getCurrentDatePlusDaysNo0Format(10)]
const recurringPODetails = ["Roebuck Removals ", "Settling in Service - Other", "USD - US Dollar", "50", "Weekly", "testDescrpt1"]

xdescribe('Settling in services test', () => { // bug Milestone progress didnt removed after removing added partner

    before(async () => {
        await passSignIn.signIn();
    });

    it('Go to Services / Settling in services page and check label', async () => {
    ...
    it('Verify Settling in services NA section', async () => {
        await settlingInServicesPage.clickSettlingInServicesToggle();
        await settlingInServicesPage.clickSettlingInServicesNaEditBtn();
        ...
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_1)).toBe(true);
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_2)).toBe(true);
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_3)).toBe(false);
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_4)).toBe(false);
        expect(await settlingInServicesPage.getSettlementSpecificsAnswerText()).toBe(requestSpecifics_1);

    });
    it('Verify edit Settling in services NA section', async () => {
        await settlingInServicesPage.clickSettlingInServicesNaEditBtn();
        ...
        await commonElements.refreshPage();
        await settlingInServicesPage.clickSettlingInServicesToggle();
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_1)).toBe(false);
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_2)).toBe(false);
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_3)).toBe(true);
        expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_4)).toBe(true);
        expect(await settlingInServicesPage.getSettlementSpecificsAnswerText()).toBe(requestSpecifics_2);
    });

    it('Verify Partner Settling in services section ', async () => {
        await commonElements.refreshPage();
        ...
        await commonElements.setChbByLabel(serviceRequests_1,true);
        await commonElements.setChbByLabel(serviceRequests_2,true);
        await commonElements.setChbByLabel(serviceRequests_3,false);
        await commonElements.setChbByLabel(serviceRequests_4,false);
        await settlingInServicesPage.setOtherSpecificsValue(requestSpecifics_1);
        ...
        expect(await commonElements.getChbState(serviceRequests_1)).toBe(true);
        expect(await commonElements.getChbState(serviceRequests_2)).toBe(true);
        expect(await commonElements.getChbState(serviceRequests_3)).toBe(false);
        expect(await commonElements.getChbState(serviceRequests_4)).toBe(false);
        expect(await settlingInServicesPage.getOtherSpecificsValue()).toBe(requestSpecifics_1);

    });
    // xit('Verify NA Labels after set partner sections data', async () => { //bug NA labels doesn't change after partner sections are updated
    //     await commonElements.refreshPage();
    //     await settlingInServicesPage.clickSettlingInServicesToggle();
    //     expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_1)).toBe(true);
    //     expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_2)).toBe(true);
    ...
    it('Verify Edit Partner Settling in services section ', async () => {
        await initiatedServicesPage.setInitiationDetails(contactPerson_2, notes_2);
        await commonElements.setChbByLabel(serviceRequests_1,false);
        await commonElements.setChbByLabel(serviceRequests_2,false);
        await commonElements.setChbByLabel(serviceRequests_3,true);
        await commonElements.setChbByLabel(serviceRequests_4,true);
        await settlingInServicesPage.setOtherSpecificsValue(requestSpecifics_2);
        await commonElements.refreshPage();
        expect(await initiatedServicesPage.getContactPersonValue()).toBe(contactPerson_1);
        expect(await initiatedServicesPage.getNotesToPartnerValue()).toBe(notes_1);
        expect(await commonElements.getChbState(serviceRequests_1)).toBe(true);
        ...
        expect(await commonElements.getChbState(serviceRequests_1)).toBe(false);
        expect(await commonElements.getChbState(serviceRequests_2)).toBe(false);
        expect(await commonElements.getChbState(serviceRequests_3)).toBe(true);
        expect(await commonElements.getChbState(serviceRequests_4)).toBe(true);
        expect(await settlingInServicesPage.getOtherSpecificsValue()).toBe(requestSpecifics_2);
    });
    // xit('Verify NA Labels after edit partner sections data', async () => {//bug NA labels doesn't change after partner sections are updated
    //     await commonElements.refreshPage();
    //     await settlingInServicesPage.clickSettlingInServicesToggle();
    //     expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_1)).toBe(false);
    //     expect(await initiatedServicesPage.getNAChbLabelIconState(serviceRequests_2)).toBe(false);
