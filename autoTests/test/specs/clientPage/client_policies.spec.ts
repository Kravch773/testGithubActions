...
describe('Policies page test', () => {
    ...
    it('Verify Add new policy form', async () => {
        ...
        expect(await policiesPage.getCoreServiceIconQty()).toBe(17);
        expect(await policiesPage.getFlexibleServiceIconQty()).toBe(17);
        expect(await policiesPage.getNotIncludedServiceIconQty()).toBe(53);
        expect(await policiesPage.getOriginCountriesQty()).toBe("2");
        expect(await policiesPage.getDestinationCountriesQty()).toBe("2");
    });
    it('Verify Edit policy form', async () => {
        await policiesPage.clickEditPolicyBtn();
        ...
        expect(await policiesPage.getCoreServiceIconQty()).toBe(17);
        expect(await policiesPage.getFlexibleServiceIconQty()).toBe(14);
        expect(await policiesPage.getNotIncludedServiceIconQty()).toBe(56);
        expect(await policiesPage.getOriginCountriesQty()).toBe("3");
        expect(await policiesPage.getDestinationCountriesQty()).toBe("3");
        await policiesPage.clickEditPolicyBtn();
        ...
        expect(await commonElements.checkPopUpMsgAndForNotDisplayed(policyUpdateMsg)).toBe(true);
        await commonElements.refreshPage();
        expect(await policiesPage.getOriginCountriesQty()).toBe(countriesQty);
        expect(await policiesPage.getDestinationCountriesQty()).toBe(countriesQty);
    });
    it('Verify remove policy', async () => {
        ...
        await commonElements.refreshPage();
        expect(await policiesPage.getPoliciesQty()).toBe(2);
    });
});
